...
router.get("/branch/:id", function (req, res) {
  ...
  var query =
    "select * from car inner join storage2 on car.storageName = storage2.storageName where car.branchId = ?";
...
router.get("/history", function (req, res) {
  ...
  var query =
    "select car.storageName,branch.branchName,details.bookingTime,details.fromTime,details.toTime from booking join details using(bookingId) join car using(carId) join branch using(branchId) where booking.userId= ? ";
...
router.get("/addStorage", function (req, res) {
  if (authenticated == false) {
    res.redirect("/adminlogin");
  } else {
    var query = "select storageName from storage2";
    con.query(query, function (err, response) {
      if (err) throw err;
...
router.post("/addStorage", function (req, res) {
  if (authenticated == false) {
    ...
    var query =
      "insert into storage2 (storageName,description,image1,image2,image3) values(?,?,?,?,?)";
    var query2 = mysql.format(query, [
      storageName,
      description,
      image1,
...
router.get("/addStorageBranch", function (req, res) {
  if (authenticated == false) {
    res.redirect("/adminlogin");
  } else {
    var query =
      "select carId,storageName,car.branchId,branchName from car join branch on branch.branchId =car.branchId";
    var query2 = mysql.format(query);
    con.query(query2, function (err, response) {
      if (err) throw err;
      res.render("addStorageBranch", {
        response: response,
      });
    });
  }
...
router.post("/addStorageBranch", function (req, res) {
  if (authenticated == false) {
    res.redirect("/adminlogin");
  } else {
    ...
    var query =
      "insert into car (carId,storageName,priceOne,priceFive,priceSix,branchId) values(?,?,?,?,?,?)";
    var query2 = mysql.format(query, [
      carId,
      storageName,
      priceOne,
...
//select user2.first,user2.last,user.userId,car.carId,details.bookingId,car.storageName,details.fromTime,details.toTime from booking join user on user.userId = booking.userId join user2 on user.driving = user2.driving join car on car.carId = booking.carId join details on details.bookingId = booking.bookingId
...
router.get("/deleteStorage", function (req, res) {
  if (authenticated == false) {
    res.redirect("/adminlogin");
  } else {
    var query =
